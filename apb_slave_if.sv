module apb_slave_if(
  ...
  always_ff @(posedge PCLK or negedge PRESETn) begin
    if (!PRESETn) begin
      ...
      // 清除中断标志
      if (PSEL && PENABLE && PWRITE && (addr == 12'h010)) begin
        if (PWDATA[0]) irq_sta_reg[0] <= 1'b0;
        if (PWDATA[1]) irq_sta_reg[1] <= 1'b0;
      end
      ...
      if (PSEL && PENABLE && PWRITE) begin
        case (addr)
          ...
          12'h00C: begin  // IRQ_EN
            irq_en_reg <= PWDATA[1:0];
          end
          12'h014: begin  // PKT_LEN_EXP
            pkt_len_exp_reg <= PWDATA[5:0];
          end
          12'h040, 12'h044, 12'h048, 12'h04C, 12'h050, 12'h054, 12'h058, 12'h05C: begin  // PKT_MEM
            if (!busy_i) begin
              pkt_mem_we_o     <= 1'b1;
          ...
          default: begin
            // 写只读寄存器或未定义地址
            if (addr < 12'h02C || (addr >= 12'h040 && addr <= 12'h05C)) begin
              // 写只读寄存器
              PSLVERR <= 1'b1;
            end else begin
              // 访问未定义地址
